...
checkyaml(){
  ...
  #检测vless/hysteria协议
  if [ -n "$(cat $yamlnew | grep -oE 'type: vless|type: hysteria')" ] && [ "$clashcore" != "clashmeta" ];then
    echo -----------------------------------------------
...
##########需要变更的配置###########
  ...
  [ -z "$(cat $clashdir/user.yaml 2>/dev/null | grep '^dns:')" ] && { 
    [ "$clashcore" = 'clashmeta' ] && dns_default_meta=', https://1.0.0.1/dns-query, https://223.5.5.5/dns-query'
